...
class Search {
      ...
      success: response => {
        ...
        if (this.resultsArr.length < 1) {
          this.requestsResultsSection.html(
            "<p class='initial-message'>Sorry! We couldn't find any matching results.</p>"
          );
        } else {
          this.requestsResultsSection.html("");
  ...
  addResult(i, resultsSection) {
    ...
    let resultDiv = $("<div />")
      .addClass("pwf-service-search-result")
      .attr("tabindex", 0);
    let resultA = $("<a />").attr(
      "href",
      siteRoot + "/service/?service=" + this.resultsArr[i]["id"]
    );
    ...
    trimmedDescription =
      trimmedDescription.length < rawDescription.length
  ...
  addResultBatch(resultsSection) {
    if (
      this.resultsArr.length <=
      parseInt(this.batchCounter, 10) + parseInt(this.batchInterval, 10)
    ) {
      ...
      for (
        let i = this.batchCounter;
        i < parseInt(this.batchCounter, 10) + parseInt(this.batchInterval, 10);
        i++
      ) {
        // if (this.resultsArr[i]['found_in'] == 'title'){
        //     this.addResult(i, resultsSection);
  ...
  loadMoreResults = () => {
    if (this.elementInView(this.loadMoreDiv) && this.moreResults == true) {
      this.moreResults = false;
      this.addResultBatch(
        this.loadMoreDiv.closest("main").find(".pwf-search-results")
      );
    }
  };
  searchServices() {
